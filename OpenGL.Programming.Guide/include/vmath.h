...
namespace vmath
{
    ...
    //These type don't exist in TGLSL and don't have full implementation
    //this is enougth to get some template functions to compile correctly
    ...
    public:
        ...
        //constructor from the element type, explicit to prevent assignment from T
        explicit inline matNM(T t)
        {
            for (int n = 0; n < w; n++) {
                data[n] = t;
            }
        ...
        //matrix multiply
        //this only works for square matrices. Need more template skill to make a non-square version.
        inline my_type operator*(const my_type& that) const
        {
            my_type result(0);
            for (int j = 0; j < w; j ++)
    ...
    protected:
        ...
        //assign function, called from assignment operator and copy constructor
